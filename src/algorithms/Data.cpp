 ...
 *
 * COMPLEXITY: O((V+E) * log(V)), where V is the number of vertices, and E the number of edges
 *
...
/**
 * @brief Applies the 3-opt algorithm to improve the given TSP path.
 *
 * The 3-opt algorithm is an iterative algorithm that attempts to improve an existing path by swapping three edges.
 * In this case, it can apply swaps to both pairs or groups of three edges (combining 2-opt with pure 3-opt approach), to provide a better solution.
 * It does this by trying to find three edges that, if swapped, would result in a shorter path.
 * If such edges are found, the swap is performed and the process is repeated until no further improvements can be made.
